$(function(){
	...
	var emailVerify = new RegExp("^[a-zA-Z0-9_.+-]+@(?:(?:[a-zA-Z0-9-]+\.)?[a-zA-Z]+\.)?(cobcreditunion)\.com$"); //Email used must be @cobcreditunion domain
	...
	$("#email").focus(function(){
		console.log("focused");
		if(!emailVerify.test($(this).val())){
			$("#emailPrompt").slideDown();
			emailMatch=false;
		}
	});
	...
	$("#confirmpassword, #email").keyup(function(){
		if(passwordStrong && passwordMatch && emailMatch){
			console.log("PasswordStrong and PasswordMatch");
			$("#submitButton").removeClass("conditionalSubmit").addClass("enabled");
		}
		else{
